...
if ($ADOBE_CLIENT_ID === "YOUR_CLIENT_ID" || $AWS_KEY === "YOUR_AWS_KEY") {
    ...
    echo json_encode([
        'success' => false, 
        'error' => "Please configure Adobe Client ID and AWS S3 keys in adobe_lightroom.php"
    ]);
    exit;
}
...
try {
    ...
    $ch = curl_init('https://image.adobe.io/lrService/autoTone');
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($ch, CURLOPT_POST, true);
    ...
    if ($apiHttpCode !== 200 && $apiHttpCode !== 202) {
        throw new Exception("Lightroom API Error: " . $res);
    }
    ...
    echo json_encode([
        ...
        'message' => 'Lightroom auto-tone applied successfully.'
    ]);
